...
class Trainer:
    ...
    def __init__(self, trainer_config: TrainerConfig):
        ...
        try:
            ...
            raise RuntimeError(f"🚨 Failed to load horizontal: {self.trainer_cfg.sd_model_horizontal}"
                               f"🔥 The real cause: {e}") from e
    ...
    def _generate_sds_reference(self, render_image: torch.Tensor, depth_map: torch.Tensor, step: int, mode: str) -> Image.Image:
    ...
    def _get_vertical_slice_mask(self, pos: torch.Tensor, viewmat: torch.Tensor, slice_thickness: float = 0.01):
        """
    ...
    def _train_vertical_views(self, epoch: int):
        ...
        for i in pbar:
            ...
            if epoch % self.trainer_cfg.sds_per_epoch == 0:
                ref_img = self._generate_sds_reference(render_image, 
            ...
            pbar.set_description(f"🎥 [VERTICAL CROSS-SECTION] {i}/30 views training! 📉LOSS: {loss.item():.5f}")
    ...
    def _train_horizontal_views(self, epoch: int):
        ...
        for i, center_pos in enumerate(pbar):
            ...
            pbar.set_description(f"🔪 [HORIZONTAL CROSS-SECTION] {i}/50 slices training! 📉LOSS: {loss.item():.5f}")
            torch.cuda.empty_cache()
    ...
    def _regularize_with_original(self):
        ...
        for i in range(30):
            ...
            pbar.set_description(f"🛡️ [TEXTURE REFINEMENT] {i}/30 For spatial consistency! 📉LOSS: {loss.item():.5f}")
